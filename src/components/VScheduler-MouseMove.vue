...
const onEventStart = (e: DragEvent, item: any) => {
  ...
  const removeEventMove = useEventListener(document, 'mousemove', (e: MouseEvent) => {
    const axisMove = {
...
const getHeight = (resource: any) => {
  return eventsSorted.value.filter(event => event.resourceId === resource.id).reduce((count, current, index, events) => {
    if (index > 0) {
    ...
    <VRow no-gutters class="h-100 overflow-auto">
      ...
      <VCol class="overflow-x-auto overflow-y-hidden" v-scroll="onScrollText">
        ...
        <VSheet :width="eventsContainerSize.width" :height="eventsContainerSize.height" class="scheluder-events-container"
          :style="gridStyle">
          <template v-for="( event, eventIndex ) in  eventsSorted" :key="event.id">
            <!-- Event Item -->
